...
const MOCK_PROGRAMS: WorkoutProgram[] = [
  {
    ...
    moves: ['Barbell Cleans (5x5)', 'Kettlebell Red-Sun Swings (4x20)', 'Weighted Pull-Ups (+15kg, 4x6)', 'Heavy Landmine Presses (3x12)']
  },
  {
    id: 'p2',
    nameKanji: '影武',
    ...
    moves: ['Snatch-Grip Deadlifts (4x6)', 'Dumbbell Renegade Rows (3x12)', 'Plyo Box Jumps (5x10)', 'Cable Woodchops (3x15)']
  },
  {
    id: 'p3',
    nameKanji: '息吹',
    ...
    moves: ['Hindu Push-Ups (4x20)', 'Cosmic Archer Squats (3x15)', 'Pistol Squats (3x8 per leg)', 'Core L-Sit Holds (4x15s)']
  },
  {
    id: 'p4',
    nameKanji: '疾風',
    ...
    moves: ['Clapping Push-Ups (4x12)', 'Explosive Jump Lunges (4x20)', 'Burpee-to-Sprawl Overload (3x15)', 'Hollow Body Rockers (4x40s)']
  }
];
                  ...
                  {MOCK_PROGRAMS
                    ...
                    .map(prog => (
                      <div 
                        ...
                        className={`border rounded-xl p-4 cursor-pointer transition-all ${selectedProgram?.id === prog.id ? 'bg-gradient-to-br from-indigo/30 via-kachi to-kachi border-rose-500/40 shadow-[0_4px_20px_rgba(255,59,48,0.15)]' : 'bg-kachi/40 border-white/5'}`}
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            <div className="flex items-center gap-2">
          ...
          {currentTab === '魂' && (
            <div className="space-y-6 pt-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-kachi/40 rounded-xl p-3.5 border border-white/5">
          ...
          <div className="w-full max-w-sm bg-kachi/30 rounded-2xl border border-white/5 p-4 space-y-3">
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {activeRunningProgram.moves.map((move, i) => (
                <div key={i} className="flex items-center gap-3 bg-void/50 p-2.5 rounded-lg border border-white/5 text-xs text-zinc-300 font-mono">
                  <span className="w-5 h-5 rounded-full bg-rose-500/10 border border-rose-500/30 flex items-center justify-center text-[10px] text-rose-500">{i+1}</span>
                  <span>{move}</span>
                </div>
              ))}
            </div>
